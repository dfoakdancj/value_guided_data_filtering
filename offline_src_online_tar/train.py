...
from value_guided_data_filtering.env.env_utils                             import call_terminal_func
from value_guided_data_filtering.env.common                                import call_env
from value_guided_data_filtering.env.sampler                               import EnvSampler
from value_guided_data_filtering.misc.utils                                import seed_all, make_exp_path, obtain_exp_path, load_from_pkl_with_decompress, save_to_pkl_with_compress
from value_guided_data_filtering.misc.logger                               import Logger
from value_guided_data_filtering.offline_src_online_tar.agent              import ValueEqualTransferOffline
...
def train(config: Dict, exp_name: str, continue_train: bool) -> False:
    ...
    if continue_train:
        exp_path        =       obtain_exp_path(config, exp_name)
        if os.path.exists(exp_path) and os.path.exists(exp_path + 'log.pkl'):
            print(f"\nFound existing exp path to support continue training \n | Now continue learning ! ! |\n")
            learning_from_scratch = False
            # load config
            with open(exp_path + 'config.yaml', 'r', encoding='utf-8') as config_loader:
                config  =   yaml.safe_load(config_loader)
            config['result_path']   =   result_path
            config['exp_path']      =   exp_path
            # load agent
            agent                   =   load_from_pkl_with_decompress(exp_path + 'agent.bt')
            agent.exp_path          =   exp_path
            # load logger
            tb              =   SummaryWriter(config['exp_path'])
            logger          =   Logger()
            logger.load(exp_path + 'log.pkl')
            # train env
            ...
            # indicators
            total_step              =   logger.data['step'][-1]
            total_step_in_tar_env   =   logger.data['step in target'][-1]
            best_train_score        =   np.max(logger.data['src env avg score'])
            best_test_score         =   np.max(logger.data['tar env avg score'])
        else:
            print(f"\nWARNING: No existing exp path to support continue training \n | Now learning from scratch ! ! |\n")
    ...
    if learning_from_scratch:
        ...
        logger  = Logger()
        # agent
        agent = ValueEqualTransferOffline(config)
        # start rollout
    ...
    while total_step <= config['max_step']:
        ...
        if total_step % config['save_freq'] == 0:
            ...
            save_to_pkl_with_compress(config['exp_path'] + 'agent.bt', agent)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env',                help='choose the environment', type=str, default='halfcheetah')
    ...
    parser.add_argument('--use_importance_weighting', help='whether use the rejection sampling or importance weighting', action='store_true')

    parser.add_argument('--continue_train', help='whether to continue learning', type=bool, default=True)
    ...
    if args.use_importance_weighting:
        config['use_reject_sampling'] = False
    else:
        config['use_reject_sampling'] = True
    ...
    config['use_pure_single_step'] = True

    for seed in args.seeds:
        config['seed'] = seed
        train(config, args.exp_name, args.continue_train)