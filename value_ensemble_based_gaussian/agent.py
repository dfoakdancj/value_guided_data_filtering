...
class DataFilteringEnsembleQ:
    def __init__(self, config: Dict) -> None:
        ...
        self.QFunction      =       QEnsemble(
            ensemble_size   =   self.model_config['value_ensemble_size'],
            s_dim           =   self.s_dim,
            a_dim           =   self.a_dim,
    ...
    def _compute_value_loss(
        ...
        with torch.no_grad():
            ...
            tar_next_sa_q_ensemble              = self.QFunction_tar(tar_next_s, tar_next_a)
            tar_next_sa_q_ensemble              = torch.stack(tar_next_sa_q_ensemble, dim=0)
            tar_next_sa_q                       = torch.min(tar_next_sa_q_ensemble, dim=0, keepdim=False).values
            tar_value_target                    = tar_r + (1 - tar_done) * self.gamma * (tar_next_sa_q - self.alpha * tar_next_a_logprob)
        tar_q_ensemble  =   self.QFunction(tar_s, tar_a)
        tar_q_loss      =   0
        for tar_q in tar_q_ensemble:
            tar_q_loss +=   F.mse_loss(tar_q, tar_value_target)
        ...
        with torch.no_grad():
            ...
            src_next_sa_q_ensemble              = self.QFunction_tar(src_next_s, src_next_a)
            src_next_sa_q_ensemble              = torch.stack(src_next_sa_q_ensemble, dim=0)
            src_next_sa_q                       = torch.min(src_next_sa_q_ensemble, dim=0, keepdim=False).values    # [batch_size, 1]
        ...
        ## 2. obtain the src value targets for comparison
            if self.use_behavior_next_a:
                src_next_sa_q_ensemble_for_compare      = self.QFunction_tar(src_next_s, src_hist_next_a)
                src_next_sa_q_ensemble_for_compare      = torch.stack(src_next_sa_q_ensemble_for_compare, dim=0)
                src_next_sa_q_for_compare               = torch.min(src_next_sa_q_ensemble_for_compare, dim=0, keepdim=False).values
        ...
        ## 3. expand the dynamics model (of target domain) for TD-h value target distribution
            tar_TD_H_value_target               = self._value_expansion(src_s, src_a, terminal_func)            # [value_ensemble_size, batch_size, 1]
        ...
        src_q_ensemble      =   self.QFunction(src_s, src_a)
        src_q_loss          =   0
        for src_q in src_q_ensemble:
            if self.use_reject_sampling:
                src_q_loss      +=   (accept_gate * (src_q - src_value_target) ** 2).mean()
            else:
                src_q_loss      +=   (weights * (src_q - src_value_target) ** 2).mean()
    ...
    def _value_expansion(self, src_s: torch.tensor, src_a: torch.tensor, terminal_func: Callable) -> torch.tensor:
        ...
        dyna_pred_mean, dyna_pred_var   =   self.dynamics.predict(inputs=torch.concat([src_s, src_a], -1), factor_ensemble=True)   # [1, batch_size, 1 + |S|]
        ...
        dyna_pred_next_s                =   src_s + dyna_pred_delta_s                                                              # [1, batch_size, |S|]
        dyna_pred_next_s                =   dyna_pred_next_s.squeeze(0)                                                            # [batch_size, |S|]
        ...
        final_q_ensemble    =   self.QFunction_tar(final_pred_next_s, final_action)
        final_q_ensemble    =   torch.stack(final_q_ensemble, dim=0)                    # [value_ensemble, batch_size, 1]
    
        TD_H_Value  =   cumulate_r + notdone * discount * final_q_ensemble
        return TD_H_Value

    def _compute_policy_loss(self, s: torch.tensor,) -> torch.tensor:
        a, a_log_prob, _    =   self.policy(s)
        q_value_ensemble    =   self.QFunction(s, a)
        q_value_ensemble    =   torch.stack(q_value_ensemble, dim=0)
        q_value             =   torch.min(q_value_ensemble, dim=0, keepdim=False).values
    ...
    def _compute_opt_policy_loss(self, s: torch.tensor,) -> torch.tensor:
        ...
        q_value_ensemble    =   self.QFunction(s, a)
        q_value_ensemble    =   torch.stack(q_value_ensemble, dim=0)
        q_value             =   torch.max(q_value_ensemble, dim=0, keepdim=False).values
        neg_entropy         =   a_log_prob
