...
from value_guided_data_filtering.value_ensemble_based_gaussian.agent               import DataFilteringEnsembleQ


def train(config: Dict, exp_name: str = None) -> False:
    config['result_path'] = f"{str(Path(__file__).parent.absolute())}/results/optimistic_ensemble_q/"
    ...
    logger  = Logger()
    # agent
    agent = DataFilteringEnsembleQ(config)
    # start rollout
...
if __name__ == '__main__':
    ...
    parser.add_argument('--value_ensemble_size',      help='dynamics ensemble size', default=3, type=int)    
    ...
    config['model_config']['value_ensemble_size']   =   args.value_ensemble_size
