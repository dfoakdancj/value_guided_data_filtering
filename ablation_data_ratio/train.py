...
from value_guided_data_filtering.misc.utils                                import seed_all, make_exp_path
from value_guided_data_filtering.misc.logger                               import Logger
from value_guided_data_filtering.ablation_data_ratio.agent                 import value_guided_data_filtering



def train(config: Dict, exp_name: str = None) -> False:
    config['result_path'] = f"{str(Path(__file__).parent.absolute())}/results/"
    ...
    logger  = Logger()
    # agent
    agent = value_guided_data_filtering(config)
    # start rollout
    ...
    while total_step <= config['max_step']:
        ...
        if config['use_behavior_next_a']:
            agent.src_buffer.store((cur_state, action, [reward], [done], next_state, agent.sample_action(next_state, with_noise=True)))
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--tar_env_interact_freq', help='size of source env samples vs size of target env samples', default=10, type=int)
    ...
    config['tar_env_interact_freq']     = args.tar_env_interact_freq
